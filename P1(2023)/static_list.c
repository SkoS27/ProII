...
tPosL findItem(tParticipantName d,tList L){
    ...
    if (L.LastPos==LNULL)
        ...
        if (strcmp(L.data[p].participantName, d) != 0) /*comparo el participantName de la posicion p con el parcicipante que queremos encontrar*/
            return LNULL;
        else
            return p;
    }
...
bool insertItem(tItemL d, tPosL p, tList *L){
    ...
    if (L->LastPos==Max-1){
        ...
        for (i = L->LastPos; i>= p+1 ; i--) {
            L->data[i]=L->data[i-1];}/*recorremos la lista desde la ultima posicion a la posicion donde vamos a insertar el nuevo participante y lo insertamos*/
        L->data[p]=d;
        return true;/*devolvemos true ya que se ha podido añadir el nuevo participante*/
    }
}
