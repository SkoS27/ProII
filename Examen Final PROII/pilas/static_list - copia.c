...
tPosL findItem(tProductId d,tList L){
    ...
    if (L.LastPos==LNULL)
        ...
        for (p = first(L); (p<=L.LastPos)&&(strcmp(d,L.data[p].productId)!=0); p++);/*mientras p sea distinto de la ultima posicion avanza desde la primera hasta la ultima*/
            if (strcmp(L.data[p].productId, d) != 0) /*comparo el productid de la posicion p con el producto que queremos encontrar*/
                return LNULL;
            else
                return p;
    }
